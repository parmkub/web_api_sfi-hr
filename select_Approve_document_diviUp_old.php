...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    END absence_hour,
        ...
        a.STATUS_APPROVE
        ORDER by creation_date ASC";
    }else if($code == '4110'){ //โรงงาน 4 ดูโครูม ด้วย เจียบ ชนะชัย
        $sql = "SELECT 
        (SELECT MAX(b.absence_date)
        FROM sf_per_absence_mobile b
        WHERE b.absence_document = a.absence_document) Max,
    ...
    END absence_hour,
        ...
        ON emp.employee_code = a.employee_code
        WHERE emp.position_group_code != '$positionGroupCode'
        AND emp.$namePosiyer in('$code','1420')
        AND a.absence_status < 2 
        AND to_char(a.creation_date,'YY')= to_char(SYSDATE,'YY')
        GROUP BY a.employee_code,
        a.absence_code,
        ...
        a.STATUS_APPROVE
        ORDER by creation_date ASC";
    }else if($code == '1220'){ //โรงงาน 1 ดูโรงงาน 3 ด้วย อรอุมา
        $sql = "SELECT 
        (SELECT MAX(b.absence_date)
        FROM sf_per_absence_mobile b
        WHERE b.absence_document = a.absence_document) Max,
    ...
    END absence_hour,
        ...
        a.STATUS_APPROVE
        ORDER by creation_date ASC";
    }else if($code == '2110'){ //โรงงาน 2 ดูโรงงาน 3 ด้วย รัชนิดา
        $sql = "SELECT 
        (SELECT MAX(b.absence_date)
        FROM sf_per_absence_mobile b
        WHERE b.absence_document = a.absence_document) Max,
